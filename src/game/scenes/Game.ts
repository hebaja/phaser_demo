...
export class Game extends Scene
{
    ...
    create ()
    {
        ...
        const terrain = map.addTilesetImage('Tilemap_color1', 'Tilemap_color1_img')
        const sky = map.addTilesetImage('sky_tile', 'sky_tile_img');
        if (!terrain || !sky) throw new Error('Error loading tileset')
        map.createLayer('background', sky, 0, 0);
        const platforms = map.createLayer('platforms', terrain, 0, 0);

        Player.createAnims(this)
        Gold.createAnims(this)
        Enemy.createAnims(this)
        this.player = new Player(this, 0, 850, 'player_idle')
		// this.enemy = new Enemy(this, 900, 900, 'enemy')

        this.golds.push(new Gold(this, 512, 880, 'gold'))
        this.golds.push(new Gold(this, 712, 880, 'gold'))
        ...
        if (!platforms) throw new Error('platorm tiles not found')
        platforms?.setCollisionByExclusion([-1])
        this.physics.add.collider(this.player, platforms)
        // this.physics.add.collider(this.golds[0], platforms)
        // this.physics.add.collider(this.golds[1], platforms)
